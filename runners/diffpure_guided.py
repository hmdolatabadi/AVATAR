...
class GuidedDiffusion(torch.nn.Module):
    def __init__(self, args, config, device=None, model_dir='pretrained/guided_diffusion', dataset='imagenet'):
        super().__init__()
        self.args = args
        self.config = config
        if device is None:
        ...
        MODEL_PATH = f'{model_dir}/256x256_diffusion_uncond.pt' if dataset == 'imagenet' else f'{model_dir}/DDPM_IP_ImageNet32.pt'
