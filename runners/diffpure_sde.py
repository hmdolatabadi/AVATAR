...
def aux_restore_checkpoint(ckpt_dir, state, device):
    loaded_state = torch.load(ckpt_dir, map_location=device)
    state['ema'].load_state_dict(loaded_state['ema'])
...
class RevVPSDE(torch.nn.Module):
    ...
    def rvpsde_fn(self, t, x, return_type='drift'):
        ...
        if return_type == 'drift':
            ...
            elif self.score_type == 'score_sde_sub':
                # model output is epsilon
                sde = sde_lib.subVPSDE(beta_min=self.beta_0, beta_max=self.beta_1, N=self.N)
                score_fn = mutils.get_score_fn(sde, self.model, train=False, continuous=True)
                score = score_fn(x_img, t)
                assert x_img.shape == score.shape, f'{x_img.shape}, {score.shape}'
                score = score.view(x.shape[0], -1)
...
class RevGuidedDiffusion(torch.nn.Module):
    def __init__(self, args, config, device=None):
        ...
        # load model
        elif config.data.dataset == 'IN32x32':
            img_shape = (3, 256, 256)
            model_dir = 'pretrained/guided_diffusion'
            model_config = model_and_diffusion_defaults()
            model_config.update(vars(self.config.model))
            print(f'model_config: {model_config}')
            model, _ = create_model_and_diffusion(**model_config)
            model.load_state_dict(torch.load(f'{model_dir}/DDPM_IP_ImageNet32.pt', map_location='cpu'))
        ...
        elif config.data.dataset == 'PoisonedCIFAR10':
            img_shape = (3, 32, 32)
            model_dir = 'pretrained/score_sde'
            print(f'model_config: {config}')
            model = mutils.create_model(config)
            ...
            ema = ExponentialMovingAverage(model.parameters(), decay=config.model.ema_rate)
            state = dict(step=0, optimizer=optimizer, model=model, ema=ema)
            restore_checkpoint(f'{model_dir}/checkpoint_poisoned_cifar10.pth', state, device)
            ema.copy_to(model.parameters())
            ...
            ema = ExponentialMovingAverage(model.parameters(), decay=config.model.ema_rate)
            state = dict(step=0, optimizer=optimizer, model=model, ema=ema)
            restore_checkpoint(f'{model_dir}/checkpoint_cifar100.pth', state, device)
            #state = dict(step=0, ema=ema)
            #aux_restore_checkpoint(f'{model_dir}/cifar100_karras_ddpmpp.pth', state, device)
            ema.copy_to(model.parameters())

        elif config.data.dataset == 'ImageNet10':
            img_shape = (3, 32, 32)
            model_dir = 'pretrained/score_sde'
            print(f'model_config: {config}')
            model = mutils.create_model(config)
            ...
            ema = ExponentialMovingAverage(model.parameters(), decay=config.model.ema_rate)
            state = dict(step=0, optimizer=optimizer, model=model, ema=ema)
            restore_checkpoint(f'{model_dir}/checkpoint_imagenet10.pth', state, device)
            #state = dict(step=0, ema=ema)
            #aux_restore_checkpoint(f'{model_dir}/imagenet10_karras_ddpmpp.pth', state, device)
            ema.copy_to(model.parameters())
            ...
            ema = ExponentialMovingAverage(model.parameters(), decay=config.model.ema_rate)
            state = dict(step=0, optimizer=optimizer, model=model, ema=ema)
            restore_checkpoint(f'{model_dir}/checkpoint_svhn.pth', state, device)
            #state = dict(step=0, ema=ema)
            #aux_restore_checkpoint(f'{model_dir}/svhn_karras_ddpmpp.pth', state, device)
            ema.copy_to(model.parameters())
